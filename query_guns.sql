...
Supply AS (
    ...
    WHERE pl.WAREHOUSE_ID = 'SHIPPING'
      AND pl.QTY > 0
      AND pl.LOCATION_ID NOT LIKE '%STAGE%'
      AND pl.LOCATION_ID NOT LIKE '%INTERNATIONAL%'
      AND LEFT(pl.LOCATION_ID, 3) BETWEEN 'R01' AND 'R09'
    GROUP BY
        pl.PART_ID,
        pl.LOCATION_ID
),
...
-- Determine Demand horizon: customer want date is either past due OR due within next 10 days
